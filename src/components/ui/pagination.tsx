  ...
  return (
    <nav aria-label="Pagination" className={cn('font-sans', className)} {...props}>
      <div className="flex flex-wrap items-center gap-2">
        <button type="button" aria-label="Previous page" disabled={currentPage === 1} onClick={() => goTo(currentPage - 1)} className="flex h-10 items-center gap-1 rounded-full bg-white px-3 text-xs font-extrabold rk-border-sm rk-shadow-sm hover:bg-[#FDE047] focus:outline-none focus:ring-4 focus:ring-black focus:ring-offset-2 disabled:pointer-events-none disabled:opacity-40">
        ...
        <div className="flex items-center gap-1" role="list">
          {pages.map((page, index) => page === 'ellipsis' ? (
            ...
            <button key={page} type="button" role="listitem" aria-label={`Page ${page}`} aria-current={page === currentPage ? 'page' : undefined} onClick={() => goTo(page)} className={cn('flex h-10 min-w-10 items-center justify-center rounded-full px-3 text-sm font-black transition-colors focus:outline-none focus:ring-4 focus:ring-black focus:ring-offset-2', page === currentPage ? 'bg-[#FDE047] rk-border-sm rk-shadow-sm' : 'bg-white hover:bg-[#BBF7D0]')}>{page}</button>
          ))}
        </div>
