          ...
          return (
            <React.Fragment key={`${index}-${String(item.label)}`}>
              ...
              <li className="flex min-w-0 items-center">
                {isCurrent || !item.href ? (
                  <span aria-current={isCurrent ? 'page' : undefined} className={cn('truncate', isCurrent ? 'text-rk-ink' : 'text-black/55')}>
                    {item.label}
                  </span>
                ) : (
                  <a href={item.href} className="truncate rounded-md px-1 py-0.5 text-black/65 underline-offset-2 hover:bg-rk-primary hover:underline focus:outline-none focus:ring-2 focus:ring-black">
                    {item.label}
                  </a>
                )}
              </li>
