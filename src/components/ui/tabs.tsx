...
export function Tabs({
  ...
  if (variant === 'pill') {
    return (
      <div
        className={cn(
          'inline-flex items-center gap-1.5 p-1.5 rounded-full bg-rk-ink text-rk-canvas rk-border rk-shadow-md relative',
          className
        )}
      >
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={cn(
                ...
                isActive ? 'text-rk-ink' : 'text-white/80 hover:text-white'
              )}
            >
              {isActive && (
                <motion.div
                  ...
                  className="absolute inset-0 bg-rk-primary rounded-full rk-border-sm rk-shadow-sm -z-10"
                />
              )}
              {tab.icon}
              <span>{tab.label}</span>
  ...
  return (
    <div className={cn('flex items-center gap-2 border-b-3 border-rk-ink pb-1 relative', className)}>
      {tabs.map((tab) => {
        const isActive = currentTab === tab.id
        return (
          <button
            ...
            className={cn(
              ...
              isActive
                ? 'bg-rk-primary text-rk-ink rk-shadow-sm -translate-y-1'
                : 'bg-rk-surface text-rk-ink/70 hover:bg-rk-canvas'
            )}
          >
            {tab.icon}
            <span>{tab.label}</span>
